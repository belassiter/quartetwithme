...
export default function Home() {
  ...
  const [minTempo, setMinTempo] = useState(50); // Default min
  const [maxTempo, setMaxTempo] = useState(200); // Default max
  const offsetRef = useRef(0); // Re-introducing offsetRef
  const [offset, setOffset] = useState(0);
  const audioTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const countOffBufferRef = useRef<AudioBuffer | null>(null);
  ...
  const performOffsetAnalysis = () => {
    ...
    // Re-apply audio playback rate
    const originalTempo = tempoRef.current || 100;
    const playbackRate = tempo / originalTempo;
    audio.playbackRate = playbackRate;
    console.log(`Audio playback rate re-applied: ${playbackRate} (Current Tempo: ${tempo}, Original Tempo: ${originalTempo})`);
  };
  ...
  useEffect(() => {
      ...
      const initOSMD = async () => {
        ...
        if (tempoRef.current) {
          ...
          // Calculate and set min/max for tempo slider
          const defaultTempo = tempoRef.current;
          setMinTempo(Math.round(defaultTempo * 0.5));
          setMaxTempo(Math.round(defaultTempo * 1.5));
